...
namespace HkIServerImpl
{
	...
	**************************************************************************************************************/
	...
	/**************************************************************************************************************
	Вызывается, когда игрок перемещает свой корабль
	**************************************************************************************************************/
	...
	void __stdcall CharacterSelect(struct CHARACTER_ID const & cId, unsigned int iClientID)
	{
		...
		try {
			...
			if (wscCharBefore.compare(wscCharname) != 0) {
				...
				LoadUserCharSettingsAudioMsg(iClientID);
				LoadUserCharSettingsChatTime(iClientID);
				...
				foreach(lstCargo, CARGO_INFO, it)
				{
					if ((*it).iCount < 0)
					{
						...
						wchar_t wszBuf[256];
						swprintf(wszBuf, L"HkIServerImpl CharacterSelect Possible cheating detected (%s)", wscCharname.c_str());
						HkMsgU(wszBuf);
	...
	void __stdcall GFGoodSell(struct SGFGoodSellInfo const &gsi, unsigned int iClientID)
	{
		...
		CHECK_FOR_DISCONNECT
			...
			foreach(lstCargo, CARGO_INFO, it)
			{
				if ((*it).iArchID == gsi.iArchID)
				{
					...
					if (abs(gsi.iCount) > it->iCount)
					{
						...
						swprintf(wszBuf, L"HkIServerImpl GFGoodSell lstCargo(abs(gsi.iCount) > it->iCount) Possible cheating detected (%s)", wszCharname);
						HkMsgU(wszBuf);
						HkBan(ARG_CLIENTID(iClientID), true);
						HkKick(ARG_CLIENTID(iClientID));
						return;
			...
			if (!bLegalSell)
			{
				...
				swprintf(wszBuf, L"HkIServerImpl GFGoodSell !bLegalSell Possible cheating detected (%s)", wszCharname);
				HkMsgU(wszBuf);
				HkBan(ARG_CLIENTID(iClientID), true);
				HkKick(ARG_CLIENTID(iClientID));
				return;
	...
	**************************************************************************************************************/
	...
	void __stdcall Login(struct SLoginInfo const &li, unsigned int iClientID)
	{
		...
		try {
			...
			// cached player id and dir id
			wstring wscDirNewPlayer;
			HkGetAccountDirName(acc, wscDirNewPlayer);
			AccountCache::Instance().AddOrUpdate(iClientID, wscDirNewPlayer);
	...
	**************************************************************************************************************/
	...
	void __stdcall TractorObjects(unsigned int iClientID, struct XTractorObjects const &p2)
	{
		...
		if (lstCargo.size() >= 160)
		{
			uint iNumItems = ((uint)p2.pArraySpaceIDEnd - (uint)p2.pArraySpaceID) >> 2;
			PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_1257") + stows(itos(lstCargo.size())) + L"/" 
				+ stows(itos(iNumItems)) + L"]");
		}
		else
		{
			ISERVER_LOG();
