...
void HkPlayerAudioMsg(uint iClientID, const char* Audio)
{
	if (ClientInfo[iClientID].bAudioMsgSounds)
	{
		pub::Audio::PlaySoundEffect(iClientID, CreateID(Audio));
	}
}
...
void HkPlayerAutoBuy(uint iClientID, uint iBaseID)
{
	...
	foreach(lstCart, AUTOBUY_CARTITEM, it4)
	{
		...
		if(iRemHoldSize < (eq->fVolume * (*it4).iCount))
		{
			...
			if(!iNewCount) {
//				PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_1258"), (*it4).wscDescription.c_str());
				continue;
			} else
		...
		if(iCash < iCost)
			PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_1259"), (*it4).wscDescription.c_str());
		else {
...
//			HkAddCargo(ARG_CLIENTID(iClientID), (*it4).iArchID, (*it4).iCount, false);
			...
			PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_1260"), (*it4).wscDescription.c_str(), (*it4).iCount, ToMoneyStr(iCost).c_str());
		}
	}
