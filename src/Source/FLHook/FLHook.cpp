...
BOOL WINAPI DllMain(HINSTANCE hinstDLL, DWORD fdwReason, LPVOID lpvReserved)
{
	...
	{ // start FLHook
		...
		// initialize localizations server
		LocalizationManager::Instance().LoadFromFile("localization.json");

		// initialize dataBase
		DatabaseManager::Instance().Init("Freelancer.db");
		DatabaseManager::Instance().InitLocalization();
	}
	return TRUE;
}
...
void FLHookInit_Pre()
{
	...
	ConPrint(L"Welcome to Dvurechensky FLHook Console (" VERSION L")\n");
