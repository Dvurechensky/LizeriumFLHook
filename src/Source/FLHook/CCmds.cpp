...
void CCmds::XPrintPlayerInfo(HKPLAYERINFO pi)
{
	...
	Print(L"Name: %s, ID: %u, IP: %s, Host: disabled, Ping: %u, Base: %s, System: %s\n", pi.wscCharname.c_str(), pi.iClientID, pi.wscIP.c_str(), pi.ci.dwRoundTripLatencyMS, pi.wscBase.c_str(), pi.wscSystem.c_str());
}
...
void CCmds::CmdGetAccsCached()
{
	RIGHT_CHECK(RIGHT_OTHER);

	std::list<std::wstring> lst = AccountCache::Instance().GetCacheList();
