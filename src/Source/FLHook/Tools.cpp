...
wstring stowsUTF8(const string& scText)
{
	int iSize = MultiByteToWideChar(CP_UTF8, 0, scText.c_str(), -1, nullptr, 0);
	wchar_t* wszText = new wchar_t[iSize];
...
long long IniGetLL(const string& scFile, const string& scApp, const string& scKey, long long iDefault)
{
	// Используем GetPrivateProfileString для получения значения как строки
	char szValue[256] = { 0 };
	GetPrivateProfileString(scApp.c_str(), scKey.c_str(), "", szValue, sizeof(szValue), scFile.c_str());

	// Проверяем, была ли получена строка
	if (szValue[0] == 0)
	{
		return iDefault; // Если строка пустая, возвращаем значение по умолчанию
	}

	// Преобразуем строку в long long
	long long result = _strtoi64(szValue, nullptr, 10);
	return result;
}
...
wstring FormatMilliseconds(int ms)
{
	...
	wstring result;
	if (minutes > 0)
		result += to_wstring(minutes) + L"м ";
	result += to_wstring(seconds) + L"с";
