...
PATCH_INFO piCommonDLL =
{
	"common.dll", 0x6260000,
	{
		...
		{0x0639C138,		&HkIEngine::_CShip_init,				4, &HkIEngine::fpOldInitCShip,		false},
		{0x0639C064,		&HkIEngine::_CShip_destroy,				4, &HkIEngine::fpOldDestroyCShip,	false},
...
void ClearClientInfo(uint iClientID)
{
	...
	ClientInfo[iClientID].bAudioMsgSounds = true;
	ClientInfo[iClientID].bChatTime = true;
	/*
...
void LoadUserCharSettingsAudioMsg(uint iClientID)
{
	...
	ClientInfo[iClientID].bAudioMsgSounds = IniGetB(scUserFile, scSection, "sounds", false);

	CALL_PLUGINS_V(PLUGIN_LoadUserCharSettings, , (uint), (iClientID));
}

/**************************************************************************************************************
load settings from flhookhuser.ini (specific to character) - bChatTime
**************************************************************************************************************/
...
void LoadUserCharSettingsChatTime(uint iClientID)
{
	...
	ClientInfo[iClientID].bChatTime = IniGetB(scUserFile, scSection, "chattimemsg", false);
