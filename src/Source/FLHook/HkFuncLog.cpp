...
void HkHandleCheater(uint iClientID, bool bBan, wstring wscReason, ...)
{
	...
	if (wscReason[0] != '#' && Players.GetActiveCharacterName(iClientID))
	{
		...
		wchar_t wszBuf2[500];
		swprintf(wszBuf2, L"HKFuncLog HkHandleCheater Possible cheating detected: %s", wscCharname.c_str());
		HkMsgU(wszBuf2);
	}
...
bool HkAddCheaterLog(const wstring &wscCharname, const wstring &wscReason)
{
	...
	fprintf(f, "%.2d/%.2d/%.4d %.2d:%.2d:%.2d HKFuncLog HkAddCheaterLog Possible cheating detected (%s) by %s(%s)(%s) [%s]\n",
		stNow->tm_mon + 1, stNow->tm_mday, stNow->tm_year + 1900, stNow->tm_hour, stNow->tm_min, stNow->tm_sec, wstos(wscReason).c_str(), wstos(wscCharname).c_str(), wstos(wscAccountDir).c_str(), wstos(wscAccountID).c_str(), wstos(wscIp).c_str());
	fclose(f);
	return true;
}
