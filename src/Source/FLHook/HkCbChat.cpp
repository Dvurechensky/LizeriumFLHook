...
void __stdcall HkCb_SendChat(uint iClientID, uint iTo, uint iSize, void *pRDL)
{
	...
	try {
		if (HkIServerImpl::g_bInSubmitChat && (iTo != 0x10004)) {
			...
			wstring wscTRADataSenderColor = L"F4E800";
			if (g_bMsg) {
				...
				wscTRADataSenderColor = L"00FF00";
				wscTRADataColor = L"7150FF"; // system chatcolor
			}
			else if (g_bMsgU) {
				wscTRADataSenderColor = L"00FF00";
				wscTRADataColor = L"F4E800"; // universe chatcolor
			}
			else if (iTo == 0x10000)
				wscTRADataColor = L"F4E800"; // universe chatcolor
			else if (iTo == 0)
				wscTRADataColor = L"F4E800"; // console
			else if (iTo == 0x10003)
