...
struct MONEY_FIX
{
	...
	bool operator==(const MONEY_FIX& mf1) const {
		return wscCharname == mf1.wscCharname && iAmount == mf1.iAmount;
	}
};
...
void LoadUserCharSettingsAudioMsg(uint iClientID);
void LoadUserCharSettingsChatTime(uint iClientID);
...
EXPORT wstring ExtractTextFromXMLTag(const std::wstring& input, const std::wstring& tag);
EXPORT wstring ExtractDataHex(const wstring& input);
...
EXPORT void HkPlayerAudioMsg(uint iClientID, const char* Audio);
...
EXPORT wstring GetLocalized(uint iClientID, const string& wscKey);

// HkUserCmd :: Database 
EXPORT bool CheckUserExistInDb(uint iClientID);
EXPORT bool AddUserInDb(uint iClientID);
...
void TeleportPointFunc(uint& iClientID);
...
// TAG: Message
EXPORT void PrintUserCmdTextColorKV(uint iClientID, const wstring& wscKey, const wstring& wscValue);
