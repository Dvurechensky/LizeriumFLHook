...
#define PRINT_OK() PrintUserCmdText(iClientID, GetLocalized("MSG_0711"));
#define PRINT_DISABLED() PrintUserCmdText(iClientID, GetLocalized("MSG_0712"));
...
EXPORT void UserCmd_Help(uint iClientID, const wstring &wscParam)
{
	...
	if(set_bPingCmd) {
		PrintUserCmdText(iClientID, GetLocalized("MSG_0713"));
		PrintUserCmdText(iClientID, GetLocalized("MSG_0714"));
	}
...
{ 
	...
	if(!iShip) {
		PrintUserCmdText(iClientID, GetLocalized("MSG_0715"));
		return;
	}
	...
	if(!iTarget) {
		PrintUserCmdText(iClientID, GetLocalized("MSG_0716"));
		return;
	}	
	...
	{
		PrintUserCmdText(iClientID, GetLocalized("MSG_0717"));
		return;
	}
...
USERCMD UserCmds[] =
{
	{ GetLocalized("MSG_0713"),					UserCmd_Ping},
	{ GetLocalized("MSG_0714"),			UserCmd_PingTarget},
};
