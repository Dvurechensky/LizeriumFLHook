		...
		for (; start != end; ++start)
		{
			if (start->second.system == cship->iSystem)
			{
				...
				if (start->second.curr_amount < start->second.required_amount)
				{
					...
					// PrintUserCmdText(iClientID, GetLocalized("MSG_0718"), needed);
				}
			}
		}
	}
		...
		if (start->second.item == gsi.iArchID)
		{
			if (start->second.curr_amount < start->second.required_amount)
			{
				...
				if (needed > gsi.iCount)
				{
					...
					PrintUserCmdText(iClientID, GetLocalized("MSG_0719"), needed);
				}
				else
				{
					PrintUserCmdText(iClientID, GetLocalized("MSG_0720"),needed);
				}
			}
		}
	}
