...
//Произвольное название патронов Hera Torpedo для разрушения JD
//uint JDDisruptAmmo = 0;

/// True, если включено логирование лута
bool set_bLogLooting = false;

// Отключение различных пользовательских функций
...
/** Код возврата, указывающий FLHook, хотим ли мы, чтобы обработка хука продолжалась. */
PLUGIN_RETURNCODE returncode;

// TODO: Выявляйте частые /stuck использовать и взорвать корабль (или что-то в этом роде).
...
void LoadSettings()
{
	...
	string scPluginCfgFile = string(szCurDir) + "\\flhook_plugins\\playercntl.ini";
	string scPluginRestartCfgFile = string(szCurDir) + "\\flhook_plugins\\playercntl_restart.ini";
	string scPluginHelpCfgFile = string(szCurDir) + "\\flhook_plugins\\playercntl_help.ini";
	...
	Restart::LoadSettingsRestart(scPluginRestartCfgFile);
	RepFixer::LoadSettings(scPluginCfgFile);
	Message::LoadSettings(scPluginCfgFile, scPluginHelpCfgFile);
...
/// Крюк для разрушения судна. Его легче зацепить, чем PlayerDeath.
...
{
	// Если репутация игрока меньше/равна -0.55 владельцу станции
	// затем отклонить запрос на стыковку
	int iSolarRep;
	...
	if (fAttitude <= -0.55f)
	{
		...
		wstring wscMsg[3] = {
			GetLocalized(iClientID, "MSG_1567"),
			GetLocalized(iClientID, "MSG_1568"),
			GetLocalized(iClientID, "MSG_1569")
		};
...
// Определение пути к файлу в каталоге учетной записи charname с предоставленным расширением.
// Полученный путь возвращается в параметре path.
	...
	int __cdecl Dock_Call(unsigned int const &iShip, unsigned int const &iDockTarget, int iCancel, enum DOCK_HOST_RESPONSE response)
	{
		...
		else
		{
			...
			if (iTypeID == OBJ_DOCKING_RING || iTypeID == OBJ_STATION)
			{
				...
				{
					// Распечатайте сообщение, когда корабль игрока пришвартовывается.
					wstring wscMsg = GetLocalized(iClientID, "MSG_1570");
					wstring typeMsg = GetLocalized(iClientID, "MSG_1543"); 
					wstring time = GetTimeString(set_bLocalTime);
					wstring playerName = (const wchar_t*)Players.GetActiveCharacterName(iClientID);
					PrintLocalUserCmdColorText(iClientID, time, typeMsg, playerName, wscMsg, set_iLocalChatRange);
				}
			}
...
namespace HkIServerImpl
{
	...
	// Загрузочный кэш отключает чтение запрещенного файла. 
	// Проверьте это вручную при входе в систему и загрузите плеер, если он заблокирован.
	...
	void __stdcall PlayerLaunch(unsigned int iShip, unsigned int iClientID)
	{
		...
		// TODO, если игрок исключен из процесса прерывания обработки.
	...
	void __stdcall JumpInComplete_AFTER(unsigned int iSystem, unsigned int iShip)
	{
		...
		// Сделайте игрока уязвимым, как только корабль прыгнет в систему.
	...
	{
		returncode = DEFAULT_RETURNCODE;
		// Сделать игрока неуязвимым, чтобы исправить JH/JG возле минных полей, иногда взрывающихся во время прыжка (в туннеле для прыжков)
	...
	void __stdcall SPObjCollision(struct SSPObjCollisionInfo const &ci, unsigned int iClientID)
	{
		...
		// Если защита от вращений отключена, ничего не делайте.
		if (set_fSpinProtectMass == -1.0f)
			return;

		// Если целью не является игрок, ничего не делайте.
		...
		// Не делайте защиту от вращения, если только пораженный корабль не большой
		if (target_mass < set_fSpinProtectMass)
			return;

		// Не делайте защиту от вращения, если пораженный корабль не в 2 раза больше атакующего
	...
	void __stdcall GFGoodBuy(struct SGFGoodBuyInfo const &gbi, unsigned int iClientID)
	{
		...
		if (Rename::IsLockedShip(iClientID, 2))
		{
			...
			if (packageInfo->iType == 1)
			{
				...
				{
					PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0946"));
					float fItemValue;
	...
	void __stdcall ReqAddItem(unsigned int goodID, char const *hardpoint, int count, float status, bool mounted, uint iClientID)
	{
		...
		/*if (Rename::IsLockedShip(iClientID, 2))
		{
			...
			if (HP == "BAY")
			{
				//PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0947"));
			}
			else
			{
				PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0948"));
				...
				returncode = SKIPPLUGINS_NOFUNCTIONCALL;
				//PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0949"));
			}
		}*/
		...
		if (Rename::IsLockedShip(iClientID, 2))
		{
			for (list<EquipDesc>::iterator item = Players[iClientID].equipDescList.equip.begin(); item != Players[iClientID].equipDescList.equip.end(); item++)
			{
				if (item->sID == slot)
				{
					...
					{
						PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0950"));
					...
					{
						PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0951"));
						wstring wsccharname = (const wchar_t*)Players.GetActiveCharacterName(iClientID);
						wstring spurdoip;
						HkGetPlayerIP(iClientID, spurdoip);
	...
	void __stdcall ReqEquipment(class EquipDescList const &eqDesc, unsigned int iClientID)
	{
		...
		if (Rename::IsLockedShip(iClientID, 2))
		{
			PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0952"));
			wstring wsccharname = (const wchar_t*)Players.GetActiveCharacterName(iClientID);
			wstring spurdoip;
			HkGetPlayerIP(iClientID, spurdoip);
		...
		if (Rename::IsLockedShip(iClientID, 2))
		{
				...
				item != Players[iClientID].lShadowEquipDescList.pFirst; item = item->next)
			{
					...
					{
						PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0953"));
						//PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0954"), item->equip.get_count());
					}
					else
					{
						PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0955"));
					}
			}

			//PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0956"));
	...
	void __stdcall ReqModifyItem(unsigned short iArchID, char const *Hardpoint, int count, float p4, bool bMounted, unsigned int iClientID)
	{
		...
		if (Rename::IsLockedShip(iClientID, 2))
		{
			if (bMounted == true)
			{
				PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0957"));
			...
			if (bMounted == false)
			{
				PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0958"));
	...
	// Экономия после трактора для предотвращения дублирования груза, потери при столкновении
	...
	// Сохранение после сброса для снижения вероятности дублирования при сбое
	...
	void __stdcall SubmitChat(struct CHAT_ID cId, unsigned long lP1, void const *rdlReader, struct CHAT_ID cIdTo, int iP2)
	{
		...
		// Если вы находитесь на базе, то сбросьте время удара по базе, если игрок разговаривает в чате, чтобы остановить удар игрока. 
		if (ClientInfo[cId.iID].iBaseEnterTime)
		{
			ClientInfo[cId.iID].iBaseEnterTime = (uint)time(0);
		}

		// Подсистема обмена сообщениями может подавлять некоторые сообщения чата.
...
USERCMD UserCmds[] =
{
	{ L"/pos",			MiscCmds::UserCmd_Pos,			L"Используйте: /pos"},
	{ L"/stuck",		MiscCmds::UserCmd_Stuck,		L"Используйте: /stuck"},
	{ L"/droprep",		MiscCmds::UserCmd_DropRep,		L"Используйте: /droprep"},
	{ L"/dice",			MiscCmds::UserCmd_Dice,			L"Используйте: /dice 1d20 | 1d20+3 | etc."},
	{ L"/roll",			MiscCmds::UserCmd_Dice,			L"Используйте: /roll 1d20 | 1d20+3 | etc." },
	{ L"/coin",			MiscCmds::UserCmd_Coin,			L"Используйте: /coin"},
	{ L"/pimpship",		PimpShip::UserCmd_PimpShip,		L"Используйте: /pimpship" },
	{ L"/showsetup",	PimpShip::UserCmd_ShowSetup,	L"Используйте: /showsetup" },
	{ L"/showitems",	PimpShip::UserCmd_ShowItems,	L"Используйте: /showitems" },
	{ L"/setitem",		PimpShip::UserCmd_ChangeItem,	L"Используйте: /setitem" },
	{ L"/renameme",		Rename::UserCmd_RenameMe,		L"Используйте: /renameme <charname> [password]" },
	{ L"/movechar",		Rename::UserCmd_MoveChar,		L"Используйте: /movechar <charname> <code>" },
	{ L"/set movecharcode",	Rename::UserCmd_SetMoveCharCode,	L"Usage: /set movecharcode <code>" },
	{ L"/restart",		Restart::UserCmd_Restart,		L"Используйте: /restart <faction>" },
	{ L"/showrestarts",	Restart::UserCmd_ShowRestarts,	L"Используйте: /showrestarts или /showrestarts <private_code>"},
	{ L"/givecash",		GiveCash::UserCmd_GiveCash,		L"Используйте: /givecash <charname> <cash> [anon] [comment] или /gc ..." },
	{ L"/gc",			GiveCash::UserCmd_GiveCash,		L"Используйте: /givecash <charname> <cash> [anon] [comment] или /gc ..." },
	{ L"/givecasht",	GiveCash::UserCmd_GiveCashTarget,		L"Используйте: /givecasht <cash> [anon] [comment] или /gct ..." },
	{ L"/changeship",	GiveCash::UserCmd_ChangeShip,		L"Используйте: /changeship [id]" },
	{ L"/cs",			GiveCash::UserCmd_ChangeShip,		L"Используйте: /cs [id] ..." },
	{ L"/delship",		GiveCash::UserCmd_DeleteShip,		L"Используйте: /delship [id]" },
	{ L"/ds",			GiveCash::UserCmd_DeleteShip,		L"Используйте: /ds [id]" },
	{ L"/gct",			GiveCash::UserCmd_GiveCashTarget,		L"Используйте: /givecasht <cash> [anon] [comment] или /gct ..." },
	{ L"/sendcash",		GiveCash::UserCmd_GiveCash,		L"Используйте: /givecash <charname> <cash> [anon] [comment]"},
	{ L"/set cashcode",	GiveCash::UserCmd_SetCashCode,	L"Используйте: /set cashcode <code>"},
	{ L"/showcash",		GiveCash::UserCmd_ShowCash,		L"Используйте: /showcash <charname> <code> или /shc ..." },
	{ L"/drc",		GiveCash::UserCmd_DrawCash,		L"Используйте: /drawcash <charname> <code> <cash> или /drc ..." },
	{ L"/shc",		GiveCash::UserCmd_ShowCash,		L"Используйте: /showcash <charname> <code> или /shc ..." },
	{ L"/drawcash",		GiveCash::UserCmd_DrawCash,		L"Используйте: /drawcash <charname> <code> <cash> или /drc ..." },
	{ L"/group",		Message::UserCmd_GroupMsg, L"Используйте: /group <message> или /g ..." },
	{ L"/g",			Message::UserCmd_GroupMsg, L"Используйте: /group <message> или /g ..." },
	{ L"/local",		Message::UserCmd_LocalMsg, L"Используйте: /local <message> или /l ...>" },
	{ L"/l",			Message::UserCmd_LocalMsg, L"Используйте: /local <message> или /l ...>" },
	{ L"/system",		Message::UserCmd_SystemMsg, L"Используйте: /system <message> или /s ..." },
	{ L"/s",			Message::UserCmd_SystemMsg, L"Используйте: /system <message> или /s ..." },
	//{ L"/invite",		Message::UserCmd_BuiltInCmdHelp, L"Используйте: /invite <charname> или /i ..."},
	//{ L"/invite$",		Message::UserCmd_BuiltInCmdHelp, L"Используйте: /invite <clientid> или /i$ ..."},
	//{ L"/join",			Message::UserCmd_BuiltInCmdHelp, L"Используйте: /join <charname> или /j"},
	{ L"/setmsg",		Message::UserCmd_SetMsg, L"Используйте: /setmsg <n> <msg text>"},
	{ L"/showmsgs",     Message::UserCmd_ShowMsgs, L""},
	{ L"/0",		    Message::UserCmd_DRMsg, L"Используйте: /n (n=0-9)"},
	{ L"/1",		    Message::UserCmd_DRMsg, L"Используйте: /n (n=0-9)"},
	{ L"/2",		    Message::UserCmd_DRMsg, L"Используйте: /n (n=0-9)"},
	{ L"/3",		    Message::UserCmd_DRMsg, L"Используйте: /n (n=0-9)"},
	{ L"/4",		    Message::UserCmd_DRMsg, L"Используйте: /n (n=0-9)"},
	{ L"/5",		    Message::UserCmd_DRMsg, L"Используйте: /n (n=0-9)"},
	{ L"/6",		    Message::UserCmd_DRMsg, L"Используйте: /n (n=0-9)"},
	{ L"/7",		    Message::UserCmd_DRMsg, L"Используйте: /n (n=0-9)"},
	{ L"/8",		    Message::UserCmd_DRMsg, L"Используйте: /n (n=0-9)"},
	{ L"/9",		    Message::UserCmd_DRMsg, L"Используйте: /n (n=0-9)"},
	{ L"/s0",		    Message::UserCmd_SMsg, L"Используйте: /sn (n=0-9)"},
	{ L"/s1",		    Message::UserCmd_SMsg, L"Используйте: /sn (n=0-9)"},
	{ L"/s2",		    Message::UserCmd_SMsg, L"Используйте: /sn (n=0-9)"},
	{ L"/s3",		    Message::UserCmd_SMsg, L"Используйте: /sn (n=0-9)"},
	{ L"/s4",		    Message::UserCmd_SMsg, L"Используйте: /sn (n=0-9)"},
	{ L"/s5",		    Message::UserCmd_SMsg, L"Используйте: /sn (n=0-9)"},
	{ L"/s6",		    Message::UserCmd_SMsg, L"Используйте: /sn (n=0-9)"},
	{ L"/s7",		    Message::UserCmd_SMsg, L"Используйте: /sn (n=0-9)"},
	{ L"/s8",		    Message::UserCmd_SMsg, L"Используйте: /sn (n=0-9)"},
	{ L"/s9",		    Message::UserCmd_SMsg, L"Используйте: /sn (n=0-9)"},
	{ L"/l0",		    Message::UserCmd_LMsg, L"Используйте: /ln (n=0-9)"},
	{ L"/l1",		    Message::UserCmd_LMsg, L"Используйте: /ln (n=0-9)"},
	{ L"/l2",		    Message::UserCmd_LMsg, L"Используйте: /ln (n=0-9)"},
	{ L"/l3",		    Message::UserCmd_LMsg, L"Используйте: /ln (n=0-9)"},
	{ L"/l4",		    Message::UserCmd_LMsg, L"Используйте: /ln (n=0-9)"},
	{ L"/l5",		    Message::UserCmd_LMsg, L"Используйте: /ln (n=0-9)"},
	{ L"/l6",		    Message::UserCmd_LMsg, L"Используйте: /ln (n=0-9)"},
	{ L"/l7",		    Message::UserCmd_LMsg, L"Используйте: /ln (n=0-9)"},
	{ L"/l8",		    Message::UserCmd_LMsg, L"Используйте: /ln (n=0-9)"},
	{ L"/l9",		    Message::UserCmd_LMsg, L"Используйте: /ln (n=0-9)"},
	{ L"/g0",		    Message::UserCmd_GMsg, L"Используйте: /gn (n=0-9)"},
	{ L"/g1",		    Message::UserCmd_GMsg, L"Используйте: /gn (n=0-9)"},
	{ L"/g2",		    Message::UserCmd_GMsg, L"Используйте: /gn (n=0-9)"},
	{ L"/g3",		    Message::UserCmd_GMsg, L"Используйте: /gn (n=0-9)"},
	{ L"/g4",		    Message::UserCmd_GMsg, L"Используйте: /gn (n=0-9)"},
	{ L"/g5",		    Message::UserCmd_GMsg, L"Используйте: /gn (n=0-9)"},
	{ L"/g6",		    Message::UserCmd_GMsg, L"Используйте: /gn (n=0-9)"},
	{ L"/g7",		    Message::UserCmd_GMsg, L"Используйте: /gn (n=0-9)"},
	{ L"/g8",		    Message::UserCmd_GMsg, L"Используйте: /gn (n=0-9)"},
	{ L"/g9",		    Message::UserCmd_GMsg, L"Используйте: /gn (n=0-9)"},
	{ L"/t0",		    Message::UserCmd_SendToLastTarget, L"Используйте: /tn (n=0-9)"},
	{ L"/t1",		    Message::UserCmd_SendToLastTarget, L"Используйте: /tn (n=0-9)"},
	{ L"/t2",		    Message::UserCmd_SendToLastTarget, L"Используйте: /tn (n=0-9)"},
	{ L"/t3",		    Message::UserCmd_SendToLastTarget, L"Используйте: /tn (n=0-9)"},
	{ L"/t4",		    Message::UserCmd_SendToLastTarget, L"Используйте: /tn (n=0-9)"},
	{ L"/t5",		    Message::UserCmd_SendToLastTarget, L"Используйте: /tn (n=0-9)"},
	{ L"/t6",		    Message::UserCmd_SendToLastTarget, L"Используйте: /tn (n=0-9)"},
	{ L"/t7",		    Message::UserCmd_SendToLastTarget, L"Используйте: /tn (n=0-9)"},
	{ L"/t8",		    Message::UserCmd_SendToLastTarget, L"Используйте: /tn (n=0-9)"},
	{ L"/t9",		    Message::UserCmd_SendToLastTarget, L"Используйте: /tn (n=0-9)"},
	{ L"/target",       Message::UserCmd_SendToLastTarget, L"Используйте: /target <message> или /t ..." },
	{ L"/t",            Message::UserCmd_SendToLastTarget, L"Используйте: /target <message> или /t ..." },
	{ L"/reply",        Message::UserCmd_ReplyToLastPMSender, L"Используйте: /reply <message> или /r .." },
	{ L"/r",            Message::UserCmd_ReplyToLastPMSender, L"Используйте: /reply <message> или /r ..." },
	{ L"/privatemsg$",	Message::UserCmd_PrivateMsgID, L"Используйте: /privatemsg$ <clientid> <messsage> или /pm ..."},
	{ L"/pm$",			Message::UserCmd_PrivateMsgID, L"Используйте: /privatemsg$ <clientid> <messsage> или /pm ..."},
	{ L"/privatemsg",	Message::UserCmd_PrivateMsg, L"Используйте: /privatemsg <charname> <messsage> или /pm ..."},
	{ L"/pm",			Message::UserCmd_PrivateMsg, L"Используйте: /privatemsg <charname> <messsage> или /pm ..."},
	{ L"/factionmsg",	Message::UserCmd_FactionMsg, L"Используйте: /factionmsg <tag> <message> или /fm ..."},
	{ L"/fm",			Message::UserCmd_FactionMsg, L"Используйте: /factionmsg <tag> <message> или /fm ..."},
	{ L"/invite",	Message::UserCmd_Invite, L"Используйте: /invite <name> или /i ..." },
	{ L"/i",			Message::UserCmd_Invite, L"Используйте: /invite <name> или /i ..." },
	{ L"/factioninvite",Message::UserCmd_FactionInvite, L"Используйте: /factioninvite <tag> или /fi ..."},
	{ L"/fi",			Message::UserCmd_FactionInvite, L"Используйте: /factioninvite <tag> или /fi ..."},
	//{ L"/lastpm",       Message::UserCmd_ShowLastPMSender, L""},
	{ L"/set chattime", Message::UserCmd_SetChatTime, L"Используйте: /set chattime [on|off]"},
	{ L"/set dietime",  Message::UserCmd_SetDeathTime, L"Используйте: /set dietime [on|off]"},
	...
	{ L"/mail",			Message::UserCmd_MailShow, L"Используйте: /mail <msgnum>"},
	{ L"/maildel",		Message::UserCmd_MailDel, L"Используйте: /maildel <msgnum>"},
	//{ L"/si",			PlayerInfo::UserCmd_ShowInfo, L"Используйте: /si me или /si + выделить цель мышью в космосе"},
	//{ L"/setinfo",		PlayerInfo::UserCmd_SetInfo, L"Используйте: /setinfo"},
	{ L"/time",			Message::UserCmd_Time, L""},
	{ L"/time*",		Message::UserCmd_Time, L""},
	//{ L"/showinfo",		PlayerInfo::UserCmd_ShowInfo, L"Используйте: /showinfo или /showinfo + выделить цель мышью в космосе"},
	{ L"/showinfo*",	PlayerInfo::UserCmd_ShowInfo, L"Используйте: /showinfo* или /showinfo + выделить цель мышью в космосе"},
	{ L"/lights",		MiscCmds::UserCmd_Lights, L"Используйте: /lights"},
	{ L"/lights*",		MiscCmds::UserCmd_Lights, L"Используйте: /lights"},
	{ L"/selfdestruct",	MiscCmds::UserCmd_SelfDestruct, L"Usage: /selfdestruct"},
	{ L"/selfdestruct*",MiscCmds::UserCmd_SelfDestruct, L"Usage: /selfdestruct"},
	{ L"/shields",		MiscCmds::UserCmd_Shields, L"Используйте: /shields"},
	{ L"/shields*",		MiscCmds::UserCmd_Shields, L"Используйте: /shields"},
	//{ L"/ss",		    MiscCmds::UserCmd_Screenshot, L"Usage: /ss"},
	//{ L"/survey",		HyperJump::UserCmd_Survey, L"Используйте: /survey"},
	{ L"/showcoords",		Message::UserCmd_ShowCoords, L"Используйте: /savecoords <n>"},
	{ L"/savecoords",		Message::UserCmd_SaveCoords, L"Используйте: /savecoords <n> <text>"},
	{ L"/c0",		Message::UserCmd_LoadCoords, L"Используйте: /cn (n=0-9)"},
	{ L"/c1",		Message::UserCmd_LoadCoords, L"Используйте: /cn (n=0-9)"},
	{ L"/c2",		Message::UserCmd_LoadCoords, L"Используйте: /cn (n=0-9)"},
	{ L"/c3",		Message::UserCmd_LoadCoords, L"Используйте: /cn (n=0-9)"},
	{ L"/c4",		Message::UserCmd_LoadCoords, L"Используйте: /cn (n=0-9)"},
	{ L"/c5",		Message::UserCmd_LoadCoords, L"Используйте: /cn (n=0-9)"},
	{ L"/c6",		Message::UserCmd_LoadCoords, L"Используйте: /cn (n=0-9)"},
	{ L"/c7",		Message::UserCmd_LoadCoords, L"Используйте: /cn (n=0-9)"},
	{ L"/c8",		Message::UserCmd_LoadCoords, L"Используйте: /cn (n=0-9)"},
	{ L"/c9",		Message::UserCmd_LoadCoords, L"Используйте: /cn (n=0-9)"},
	//{ L"/setcoords",	HyperJump::UserCmd_SetCoords, L"Используйте: /setcoords"},
	//{ L"/jump",			HyperJump::UserCmd_ActivateJumpDrive, L"Используйте: /jump"},
	//{ L"/jump*",		HyperJump::UserCmd_ActivateJumpDrive, L"Используйте: /jump"},
	//{ L"/beacon",		HyperJump::UserCmd_DeployBeacon, L"Используйте: /beacon"},
	//{ L"/beacon*",		HyperJump::UserCmd_DeployBeacon, L"Используйте: /beacon"},
	//{ L"/jumpbeacon",		HyperJump::UserCmd_JumpBeacon, L"Используйте: /jumpbeacon"},
	//{ L"/jumpbeacon*",		HyperJump::UserCmd_JumpBeacon, L"Используйте: /jumpbeacon"},
	//{ L"/charge",		HyperJump::UserCmd_ChargeJumpDrive, L"Используйте: /charge"},
	//{ L"/charge*",		HyperJump::UserCmd_ChargeJumpDrive, L"Используйте: /charge"},
	//{ L"/jumpsys",		HyperJump::UserCmd_ListJumpableSystems, L"Используйте: /jumpsys"},
	//{ L"/showscan",		SystemSensor::UserCmd_ShowScan, L"Используйте: /showscan или /scan <charname>"},
	//{ L"/showscan$",	SystemSensor::UserCmd_ShowScan, L"Используйте: /showscan$ или /scanid <clientid>"},
	//{ L"/scan",			SystemSensor::UserCmd_ShowScan, L"Используйте: /showscan или /scan <charname>"},
	//{ L"/scanid",		SystemSensor::UserCmd_ShowScan, L"Используйте: /showscan$ или /scanid <clientid>"},
	//{ L"/net",			SystemSensor::UserCmd_Net, L"Используйте: /net [all|jumponly|off]"},
	{ L"/maketag",		Rename::UserCmd_MakeTag, L"Используйте: /maketag <tag> <master password> <description>"},
	{ L"/droptag",		Rename::UserCmd_DropTag, L"Используйте: /droptag <tag> <master password>"},
	{ L"/settagpass",	Rename::UserCmd_SetTagPass, L"Используйте: /settagpass <tag> <master password> <rename password>"}
};
...
{
	returncode = DEFAULT_RETURNCODE;
	PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0959"));
	PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0960"));
	PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0961"));
	PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0962"));
	PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0963"));

	if (!set_bEnableRenameMe)
	{
		PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0964"));
	}

	if (!set_bEnableMoveChar)
	{
		PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0965"));
		PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0966"));
	}

	if (!set_bEnableRestart)
	{
		PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0967"));
		PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0968"));
	}

	if (set_bEnableGiveCash)
	{
		PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0969"));
		PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0970"));
		PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0971"));
		PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0972"));
	}

	PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0973"));
	PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0974"));
	PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0975"));
	PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0976"));
	PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0977"));
	PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0978"));
	PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0979"));
	PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0980"));
	PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0981")),
	PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0982")),
	PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0983")),
	PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0984"));
	PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0985"));
	PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0986"));
	PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0987"));
	PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0988"));
}
...
/** Обратный вызов команды помощи администратора */
