...
#define MAX_CHARACTERS 1000
	...
	bool PlayerInfo::UserCmd_ShowInfo(uint iClientID, const wstring &wscCmd, const wstring &wscParam, const wchar_t *usage)
	{
		...
		if (!wszTargetName)
		{
			PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_1134"));
			return true;
		}
		...
		wstring wscNoteXML = IniGetLongWS(scFilePath, "Info", "AdminNote", L"");

		if (wscNoteXML.length())
		{
			wscPlayerInfo += L"<TEXT>" + wscNoteXML + L"</TEXT><PARA/><PARA/>";
			wscPlayerInfo += L"<POP/></RDL>";
		}		

		if (wscPlayerInfo.length() < 30)
		{
			PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_1135"));
			return true;
		}
	...
	bool PlayerInfo::UserCmd_SetInfo(uint iClientID, const wstring &wscCmd, const wstring &wscParam, const wchar_t *usage)
	{
		...
		if (iPara > 0 && iPara <= MAX_PARAGRAPHS && wscCommand == L"a")
		{
			...
			{
				PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_1136"), MAX_CHARACTERS);
				return false;
			}

			wstring wscNewMsg = IniGetLongWS(scFilePath, "Info", itos(iPara), L"") + XMLText(wscMsg);
			IniWriteW(scFilePath, "Info", itos(iPara), wscNewMsg);
			PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_1137"), length, MAX_CHARACTERS);
		}
		else if (iPara > 0 && iPara <= MAX_PARAGRAPHS && wscCommand == L"d")
		{
			IniWriteW(scFilePath, "Info", itos(iPara), L"");
			PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_1138"));
		}
		else
		{
			PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_1139"));
			PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_1140"));
			PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_1141"), MAX_PARAGRAPHS);
			PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_1142"));
		}
