...
bool extern set_bEnableRestart;
...
namespace Restart
{
	struct RestartPublicItem
	{
		string fileName;
		string factionName;
	};
	struct RestartPrivateItem
	{
		string fileName;
		string password;
		string factionName;
	...
	void LoadSettings(const string &scPluginCfgFile);
	void LoadSettingsRestart(const string &scPluginRestartCfgFile);
...
namespace GiveCash
{
	...
	void PlayerLaunch(unsigned int iShip, unsigned int iClientID);
	void BaseEnter(unsigned int iBaseID, unsigned int iClientID);
	void MoveClient(unsigned int iClientID, unsigned int targetBase, const wstring& wscCharname);
	uint HKGetLastBasePlayer(const wstring& wscCharname);
	...
	bool UserCmd_ChangeShip(uint iClientID, const wstring &wscCmd, const wstring &wscParam, const wchar_t *usage);
	bool UserCmd_DeleteShip(uint iClientID, const wstring& wscCmd, const wstring& wscParam, const wchar_t* usage);
	bool SaveAndResetShip(uint iClientID, const wstring& wscCharname, const string& scUserFileShips);
	bool ChangeShip(uint iClientID, const wstring& wscCharname, const string& scUserFileShips, int idChangeShip);
...
namespace Message
{
	void LoadSettings(const string &scPluginCfgFile, const string& scPluginCfgHelpFile);
	void ClearClientInfo(unsigned int iClientID);
	void Timer();
