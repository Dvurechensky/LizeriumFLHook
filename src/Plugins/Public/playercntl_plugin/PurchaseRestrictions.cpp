...
{
	/// Если истинно, то ID используется для проверки действительности IDs
	static bool set_bCheckIDRestrictions = false;

	/// используется для проверки действительности ID используется для проверки действительности IDs
	static bool set_bEnforceIDRestrictions = false;

	/// Сообщение, если в покупке судна отказано
	static wstring set_wscShipPurchaseDenied = L"";

	/// Сообщение, если в покупке отказано
	static wstring set_wscGoodPurchaseDenied = L"";

	/// Список элементов, по которым мы регистрируем переводы
	...
	/** Карта товаров, которые нельзя покупать без нужного товара. */
	...
	/** Карта корабля, которую нельзя покупать без нужного предмета. */
	...
	/// Регистрируйте интересующие нас элементы, чтобы мы могли видеть, какие грузы используют люди.
	...
	void PurchaseRestrictions::LoadSettings(const string &scPluginCfgFile)
	{
		...
		set_wscShipPurchaseDenied = stows(IniGetS(scPluginCfgFile, "PurchaseRestrictions", "ShipPurchaseDeniedMsg", "Вы не можете купить это, потому как на вашем корабле не установлено требуемое удостоверение!"));
		set_wscGoodPurchaseDenied = stows(IniGetS(scPluginCfgFile, "PurchaseRestrictions", "GoodPurchaseDeniedMsg", "Вы не можете купить это, потому как на вашем корабле не установлено требуемое удостоверение!"));
		...
		if (ini.open(scPluginCfgFile.c_str(), false))
		{
			...
			{
				// Ознакомьтесь с товарами, которые не покупаются list (base-nick = good-nick)
				...
				// Чтение списка элементов журнала (item-nick)
	...
	/// Убедитесь, что этот клиент имеет разрешение на покупку/монтаж этого оборудования или корабля
	/// Возвращаем true, если оборудование смонтировано так, чтобы это можно было сделать.
	...
	bool PurchaseRestrictions::GFGoodBuy(struct SGFGoodBuyInfo const &gbi, unsigned int iClientID)
	{
		...
		/// Проверьте, нет ли этого товара в списке непокупаемых.
		...
		for (mapnobuy_map_iter_t i = start; i != end; i++)
		{
			if (gbi.iGoodID == i->second)
			{
				...
				PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_1143"));
				return true;
			}
		}

		/// Проверьте ограничения для идентификатора, который есть у игрока.
		...
		// Проверьте наличие ограничений на покупку кораблей.
		if (set_bCheckIDRestrictions)
		{
		...
		if (packageInfo->iType == 3)
		{
			...
			if (hullInfo->iType == 2)
			{
				...
				{
					PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_1144"));
					pub::Player::SendNNMessage(iClientID, pub::GetNicknameId("info_access_denied"));
