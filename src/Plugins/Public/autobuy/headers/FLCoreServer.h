...
{
	int iDunno[3];
	// Это указывает на начало массива идентификаторов пространств.
	int *pArraySpaceID;
	// Это указывает на конец массива идентификаторов пространств.
...
/// <summary>
/// Это совпадает с тем, что я обнаружил в Packets(откуда я взял названия).
/// Я протестировал его с использованием шилдной батареи, 
/// где он дал 10 за объект; 0x32 для HPID; и 3 для quantity, отсюда и комментарий (я использовал только один).
/// </summary>
struct SSPUseItem
{
