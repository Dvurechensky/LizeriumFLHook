...
#define PRINT_OK() PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0679"));
#define PRINT_DISABLED() PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0680"));
...
void Condata::UserCmd_Help(uint iClientID, const wstring &wscParam)
{
	...
	if (set_bPingCmd) {
		PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0681"));
		PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0682"));
	}
...
void TimerUpdatePingData()
{
	...
	while (pPD = Players.traverse_active(pPD)) {
		...
		if (ConData[iClientID].lstPing.size() >= set_iPingKickFrame) {
			...
			unsigned int iAveragePing = 0;
			ConData[iClientID].iPingFluctuation = 0;
			foreach(ConData[iClientID].lstPing, uint, it) {
				iAveragePing += (*it);
			}

			iAveragePing /= (uint)ConData[iClientID].lstPing.size(); //  Считаем средний пинг

			foreach(ConData[iClientID].lstPing, uint, it) {
				// Сравниваем с средним,  не с предыдущим,  и берем  абсолютное значение
				ConData[iClientID].iPingFluctuation += std::abs(static_cast<int>(*it) - static_cast<int>(iAveragePing));
			}

			ConData[iClientID].iPingFluctuation /= (uint)ConData[iClientID].lstPing.size();
			ConData[iClientID].iAveragePing = iAveragePing; //  Обновляем средний пинг
		}
...
void Condata::SPObjUpdate(struct SSPObjUpdateInfo const &ui, unsigned int iClientID)
{
	...
	//ConPrint(L"SEND: " + to_wstring(iClientID));
	if (!ins)
		return; // ??? 8[
...
bool Condata::UserCmd_Ping(uint iClientID, const wstring &wscCmd, const wstring &wscParam, const wchar_t *usage)
{
	...
	PrintUserCmdTextColorKV(iClientID, GetLocalized(iClientID, "MSG_1353"), to_wstring(set_iPingKickFrame));

	Response += L"Пинг: ";
	if (ConData[iClientIDTarget].lstPing.size() < set_iPingKickFrame)
		Response += L"н/а Колеблется: н/а ";
	else {
...
bool Condata::UserCmd_PingTarget(uint iClientID, const wstring &wscCmd, const wstring &wscParam, const wchar_t *usage)
{
	...
	if (!iShip) {
		PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0683"));
		return true;
	}
	...
	if (!iTarget) {
		PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0684"));
		return true;
	}
	...
	{
		PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0685"));
		return true;
	}
