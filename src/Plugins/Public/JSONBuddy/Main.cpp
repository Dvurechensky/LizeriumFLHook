...
{
	//PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0686"));
}

void __stdcall PlayerLaunch(unsigned int iShip, unsigned int client)
{
	Condata::PlayerLaunch(iShip, client);
	//PrintUserCmdText(client, GetLocalized(iClientID, "MSG_0686"));
}
	...
	for (list<EquipDesc>::iterator item = Players[iClientID].equipDescList.equip.begin(); item != Players[iClientID].equipDescList.equip.end(); item++)
	{
		if (mapIDs.find(item->iArchID) != mapIDs.end())
		{
			if (item->bMounted)
			{
				...
				//PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0688"), stows(mapIDs[item->equip.iArchID]).c_str());
...
void HkTimerJSON()
{
	...
	else if (jsontimer == 1)
	{
		...
		//dump to a file
		FILE* file = fopen("C:/player_status.json", "w");
		if (file)
		{
