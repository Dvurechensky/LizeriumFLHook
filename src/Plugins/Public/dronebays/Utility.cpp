...
void Utility::DeployDrone(uint iClientID, const DroneBuildTimerWrapper& timerWrapper)
{
	...
	if (!iShip)
	{
		PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0619"));
		ConPrint(L"DRONEBAY: Игрок %s запустил Utility::DeployDrone в доке! Сообщите об этом разработчику FLHook - Dvurechensky \n");
		AddLog("DRONEBAY: Игрок %s запустил Utility::DeployDrone в доке! Сообщите об этом разработчику FLHook - Dvurechensky \n");
		return;
	}

	Utility::CreateNPC(iClientID, shipPos, shipRot, shipSys, timerWrapper.reqDrone);
	PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0620"));

	// Log event
	const wstring charname = reinterpret_cast<const wchar_t*>(Players.GetActiveCharacterName(iClientID));
	wstring logString = L"Игрок %s запустил дрон";
	logString = ReplaceStr(logString, L"%s", charname);
	LogEvent(wstos(logString).c_str());

	// Print launch event to surrounding players
	wstring wscMsg = L"%user запустил дрон";
