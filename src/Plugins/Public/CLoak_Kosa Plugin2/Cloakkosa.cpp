...
void HkForceDeactivateAllEquips(uint iClientID);
struct CLOAK_INFO
{
	uint		iCloakingTime;
	uint		iCloakWarmup;
...
//{
//	PrintUserCmdText(iClientID, GetLocalized("MSG_0534"));
//	PrintUserCmdText(iClientID, GetLocalized("MSG_0535"));
//	PrintUserCmdText(iClientID, GetLocalized("MSG_0536"));
//	PrintUserCmdText(iClientID, GetLocalized("MSG_0537"));
//}
...
HK_ERROR HkCloak(uint iClientID)
{
	...
	PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0538"));
	if (set_bDropShield)
...
HK_ERROR HkUnCloak(uint iClientID)
{
	...
	Plugin_Communication(CLIENT_CLOAK_INFO, &communicationInfo);
	ConPrint(L"DEBUG: uncloaking %u with equip %u\n", iClientID, ActivateEq.sID);
	//HkForceDeactivateAllEquips(iClientID);
	return HKE_OK;
}
	...
	foreach(lstEquipment, CARGO_INFO, it)
	{
		if (it->bMounted)
		{
			...
			foreach(set_lstCloakDevices, INISECTIONVALUE, it2)
			{
				...
				if (it->bMounted && it->iArchID == iArchIDCloak)
				{
...
void HkForceDeactivateAllEquips(uint iClientID)
{
	...
	foreach(lstEquipment, CARGO_INFO, it)
	{
		if (it->bMounted)
		{
			XActivateEquip ActivateEq;
			ActivateEq.iSpaceID = iShip;
			ActivateEq.sID = it->iID; // всё подряд
			ActivateEq.bActivate = false; // выключаем
			Server.ActivateEquip(iClientID, ActivateEq);
			ConPrint(L"DEBUG: uncloaking %u with equip %u\n", iClientID, ActivateEq.sID);
		}
	}
...
namespace HkIServerImpl
{
	...
	EXPORT void __stdcall SPObjUpdate(struct SSPObjUpdateInfo const &ui, unsigned int iClientID)
	{
		...
		if (CloakInfo[iClientID].bCanCloak && CloakInfo[iClientID].bMustSendUncloak && !CloakInfo[iClientID].bIsCloaking) {
			HkUnCloak(iClientID);
			CloakInfo[iClientID].bMustSendUncloak = false;
			ConPrint(L"DEBUG: SPObjUpdate %u with equip \n", iClientID);
		}
	}
