		...
		while (ini.read_header())
		{
			if (ini.is_header("DamageAdjust"))
			{
				while (ini.read_value()) 
					//;fighter freighter transport gunboat cruiser battlcruiser battleship solar
					// [DamageAdjust]
					// gold_eagle_gun01_ammo = 0.01, 0.01, 0.01, 0.01, 0.01, 0.01, 0.01, 0.01
				{
...
bool UserCmd_SnacClassic(uint iClientID, const wstring &wscCmd, const wstring &wscParam, const wchar_t *usage)
{
	...
	//if (!baseID)
	//{
	//	PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0110"));
	//	return true;
	//}
	...
	//	PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0111"), iSNACs);
	//}
	//else
	//{
	//	PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0112"));
	//}
	//PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0112"));
	return true;
}
	...
	if (iDmgToSpaceID && iDmgMunitionID)
	{
		...
		if (iter != mapDamageAdjust.end())
		{
			...
			if (subObjID == 1) // 1 is base (hull)
				...
				return; // Если попало смонтированное оборудование - не продолжайте работу с неинициализированными переменными.

			uint iTargetType;
			pub::SpaceObj::GetType(iDmgToSpaceID, iTargetType);

			// Вывод: если не истребитель и не грузовой корабль, то это явно солнечный объект.
			...
			// Исправлена ошибка с неправильным временем восстановления щита.
			if (setHealth < 0)
				setHealth = 0;

			// Исправьте ошибку с неверным сообщением о смерти.
			if (iDmgTo && subObjID == 1)
				ClientInfo[iDmgTo].dmgLast = *dmg;

			// Добавьте запись урона вместо FLHook Core.
