...
void Siege::SiegeGunDeploy(uint iClientID, const wstring &args)
{
	...
	{
		PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0391"));
		return;
	}
	...
	if (!ship)
	{
		PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0392"));
		return;
	}
	...
	{
		PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0393"));
		return;
	}
	...
	{
		PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0394"));
		return;
	}
	...
	for (map<uint, uint>::iterator i = construction_items.begin(); i != construction_items.end(); ++i)
	{
		...
		{
			PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0395"));
			for (i = construction_items.begin(); i != construction_items.end(); ++i)
			{
				const GoodInfo *gi = GoodList::find_by_id(i->first);
				if (gi)
				{
					PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0396"), i->second, HkGetWStringFromIDS(gi->iIDSName).c_str());
				}
			}
			return;
		}
	...
	PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0397"));
	PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0398"), password.c_str());
}
