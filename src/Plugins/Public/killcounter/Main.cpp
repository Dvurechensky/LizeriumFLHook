...
{
	PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0690"));
	PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0691"));
}
...
void UserCmd_Kills(uint iClientID, const wstring &wscParam)
{
	...
	if (!wscClientID.length())
	{
		...
		//PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0692"), iNumKills);
		...
		//		if (!ship)
		//			continue;
		//		PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0693"), HkGetWStringFromIDS(ship->iIdsName).c_str(), count);
		//	}
		//}
		...
		key += wstos(GetLocalized(iClientID, "MSG_1541"));
		key += "</TEXT>";
		string val = "<TRA data='0x2093FF08' mask='-1'/><TEXT>";
		val += std::to_string(iNumKills);
		val += "</TEXT>";
		wstring allCountKillsStr = stows(key) + L" " + stows(val);
		HkFMsg(iClientID, allCountKillsStr);

		key = "<TRA data='0xFF24D700' mask='-2'/><TEXT>";
		key += wstos(GetLocalized(iClientID, "MSG_1542"));
		key += "</TEXT>";
		val = "<TRA data='0x2093FF08' mask='-1'/><TEXT>";
		val += wstos(mainrank.c_str());
		val += "</TEXT>";
		allCountKillsStr = stows(key) + L" " + stows(val);
		HkFMsg(iClientID, allCountKillsStr);

		//PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0694"), iNumKills);
		//PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0695"), mainrank.c_str());
		//PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0696"));
		return;
	}
	...
	{
		PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0697"));
		//PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0696"));
		return;
	}
	HkReadCharFile(wscClientID, lstLines);
	pub::Player::GetNumKills(iClientIDPlayer, iNumKills);
	//PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0699"), iNumKills);
	//foreach(lstLines, wstring, str)
	//{
	//	if (!ToLower((*str)).find(L"ship_type_killed"))
	//	{
...
void UserCmd_Kills$(uint iClientID, const wstring &wscParam)
{
	...
	{
		PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0704"));
		//PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0696"));
		return;
	}
	wstring wscClientID = ((const wchar_t*)Players.GetActiveCharacterName(iClientIDTarget));
	HkReadCharFile(wscClientID, lstLines);
	pub::Player::GetNumKills(iClientIDTarget, iNumKills);
	//PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0706"), iNumKills);
	//foreach(lstLines, wstring, str)
	//{
	//	if (!ToLower((*str)).find(L"ship_type_killed"))
	//	{
