				...
				while (ini.read_value())
				{
					...
					while (scShipOrEquip.size())
					{
						...
						else if (Archetype::GetEquipment(iItemID))
						{
							...
							Archetype::Gun *gunEquip = (Archetype::Gun *)eq;
							EQ_TYPE eqType = HkGetEqType(eq);
							if (eqType == ET_GUN && gunEquip->iProjectileArchID && gunEquip->iProjectileArchID != 0xBAADF00D && gunEquip->iProjectileArchID != 0x3E07E70)
							{
								pb.lstAmmo.push_back(gunEquip->iProjectileArchID);
							}
							else
							{
...
namespace HkIServerImpl
{
				...
				try
				{
					...
					if (cfield->near_field(vPos) && zone && zone->lootableZone)
					{
						...
						if (ammolst == cd.mapLootAmmoLst.end())
						{
							bNoMiningCombo = true;
							if (cd.iDebug)
								PrintUserCmdText(iClientID, GetLocalized("MSG_0763"));
						}
						...
						else if (find(ammolst->second.begin(), ammolst->second.end(), ci.iProjectileArchID) == ammolst->second.end())
						{
							bNoMiningCombo = true;
							if (cd.iDebug)
								PrintUserCmdText(iClientID, GetLocalized("MSG_0764"));
						}
						...
						float iLootCount = (fRand * set_fGenericFactor * fZoneBonus * fPlayerBonus * 7);
						uint wtfLootCount = (uint)(round(iLootCount));
						

						// Remove this lootCount from the field
						set_mapZoneBonus[zone->iZoneID].fCurrReserve -= wtfLootCount;
						set_mapZoneBonus[zone->iZoneID].fMined += wtfLootCount;
						...
						{
							PrintUserCmdText(iClientID, GetLocalized("MSG_0765"), fRand, set_fGenericFactor, fZoneBonus, fPlayerBonus, wtfLootCount, iLootID, iCrateID, zone->lootableZone->dynamic_loot_count2);
							PrintUserCmdText(iClientID, GetLocalized("MSG_0766"),
								fRand, set_fGenericFactor, fPlayerBonus, fZoneBonus, wtfLootCount, iLootID, iCrateID, set_mapZoneBonus[zone->iZoneID].fCurrReserve);
						}
						...
						if (wtfLootCount)
						{
							...
							if (fHoldRemaining < wtfLootCount)
							{
								wtfLootCount = round(fHoldRemaining);
							}
							if (wtfLootCount == 0)
							{
							...
							pub::Player::AddCargo(iSendToClientID, iLootID, wtfLootCount, 1.0, false);
						}
						return;
					}
				}
	...
	EXPORT void __stdcall MineAsteroid(uint iClientSystemID, class Vector const &vPos, uint iCrateID, uint iLootID, uint iCount, uint iClientID)
	{
		returncode = SKIPPLUGINS_NOFUNCTIONCALL;
		return;
	}
...
EXPORT bool ExecuteCommandString_Callback(CCmds* cmd, const wstring &wscCmd)
{
	returncode = DEFAULT_RETURNCODE;
...
EXPORT PLUGIN_INFO* Get_PluginInfo()
{
	...
	p_PI->lstHooks.push_back(PLUGIN_HOOKINFO((FARPROC*)&HkIServerImpl::MineAsteroid, PLUGIN_HkIServerImpl_MineAsteroid , 11));
