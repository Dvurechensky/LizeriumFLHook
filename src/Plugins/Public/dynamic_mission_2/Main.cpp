...
__declspec(naked) void _HkActTrigger()
{
	__asm
	{
		...
		call HkActTrigger
		pop ecx
		// оригинальные инструкции по функциям
		push ebx
		push esi
		mov esi, [esp + 8 + 4]
		push edi
...
{
	// hack server.dll поэтому он не вызывает создание солнечного пакета и отправку
	...
	if (GetShipInspect(iSpaceID, inspect, dunno))
	{
		...
		// каждому игроку в одной системе отправить пакет создания солнечной энергии
...
EXPORT void LoadSettings()
{
	...
	// перехватить создание солнечной энергии, чтобы исправить ошибку в MP, из-за которой не отправляется загрузка
