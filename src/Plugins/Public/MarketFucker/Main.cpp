		...
		while (ini.read_header())
		{
			if (ini.is_header("BaseGood"))
			{
				while (ini.read_value())
				{
					...
					uint bastilleguard = CreateID("AD02_01_Base");
					uint currentbase;
					if (ini.is_value("base"))
					{
						currentbase = CreateID(ini.get_value_string(0));
...
void __stdcall GFGoodSell(struct SGFGoodSellInfo const &gsi, unsigned int iClientID)
{
	...
	// проверяйте только оборудование
	if (packageInfo->iType == 1)
	{
		uint iBase;
		pub::Player::GetBase(iClientID, iBase);

		//в этом случае эффективнее сначала проверить, находится ли этот объект под наблюдением.
		for (map<uint, string>::iterator iter = mapACItems.begin(); iter != mapACItems.end(); iter++)
		{
			if (iter->first == gsi.iArchID)
			{
				//PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0748"), stows(iter->second).c_str());
				//Мы перебираем имена баз, чтобы увидеть, является ли это базой, отличной от POB.
				...
				while (i != mapACBases.end())
				{
					if (*i == iBase)
					{
						...
						//PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0749"));
						// проверьте, находится ли этот товар уже под наблюдением, если да, увеличьте сумму на 1
						if (mapACSales[iClientID].items.find(gsi.iArchID) != mapACSales[iClientID].items.end())
						{
							...
							//PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0750"), mapACSales[iClientID].items.find(gsi.iArchID)->second);
...
void __stdcall GFGoodBuy(struct SGFGoodBuyInfo const &gbi, unsigned int iClientID)
{
	...
	{
	PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0751"));
	}
	*/
	if (packageInfo->iType == 1)
	{
		//PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0752"));
		uint iBase;
		pub::Player::GetBase(iClientID, iBase);
		...
		for (map<uint, string>::iterator iter = mapACItems.begin(); iter != mapACItems.end(); iter++)
		{
			...
			{
				//PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0753"));
				//Мы перебираем имена баз, чтобы увидеть, является ли это базой, отличной от POB.
				list<uint>::iterator i = mapACBases.begin();
				while (i != mapACBases.end())
				{
					if (*i == iBase)
					{
						if (mapACSales[iClientID].items.find(gbi.iGoodID) != mapACSales[iClientID].items.end())
						{
							...
							//PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0754"));
							aminiceitem = true;
							wearecool = 1;
							...
							//PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0755"), mapACSales[iClientID].items.find(gbi.iGoodID)->second);
							...
							if (mapACSales[iClientID].items.find(gbi.iGoodID)->second == 0)
							{
								...
								//PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0756"));
							}
						...
						if (wearecool == 0)
						{
							//PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0757"));
							aminiceitem = false;
			...
			if (aminiceitem == false)
			{
				...
				wstring wscMsgU = L"MF: %name был навсегда заблокирован. (Type 2)";
				wscMsgU = ReplaceStr(wscMsgU, L"%name", wscCharname.c_str());

				HkMsgU(wscMsgU);

				wstring wscMsgLog = L"<%sender> был навсегда заблокирован за попытку купить незаконный товар <%item> c базы <%basename> (см. DSAM)";
				wscMsgLog = ReplaceStr(wscMsgLog, L"%sender", wscCharname.c_str());
				wscMsgLog = ReplaceStr(wscMsgLog, L"%basename", wscBaseName.c_str());
	...
	else if (packageInfo->iType == 3)
	{
		uint iBase;
		pub::Player::GetBase(iClientID, iBase);
		//PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0758"));
		...
		for (map<uint, list<uint>>::iterator iter = mapACShips.begin(); iter != mapACShips.end(); iter++)
		{
			if (iter->first == iBase)
			{
				//PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0759"));
				// мы проверяем, является ли один из трех продаваемых здесь пакетов правильным
				list<uint>::iterator i = iter->second.begin();
				while (i != iter->second.end())
				{
					if (*i == gbi.iGoodID)
					{
						//PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0760"));
						aminiceship = true;
						break;
					}
					i++;
		...
		if (aminiceship == false)
		{
			...
			pub::Player::SendNNMessage(iClientID, pub::GetNicknameId("nnv_anomaly_detected"));
			wstring wscMsgU = L"MF: %name был навсегда заблокирован. (Type 1)";
			wscMsgU = ReplaceStr(wscMsgU, L"%name", wscCharname.c_str());

			HkMsgU(wscMsgU);

			wstring wscMsgLog = L"<%sender> был навсегда заблокирован за попытку купить незаконное судно с базы <%basename> (см. DSAM)";
			wscMsgLog = ReplaceStr(wscMsgLog, L"%sender", wscCharname.c_str());
			wscMsgLog = ReplaceStr(wscMsgLog, L"%basename", wscBaseName.c_str());
...
{
	//PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0761"));
	ClearClientInfo(iClientID);
}

void __stdcall PlayerLaunch(unsigned int iShip, unsigned int client)
{
	//PrintUserCmdText(client, GetLocalized(iClientID, "MSG_0761"));
