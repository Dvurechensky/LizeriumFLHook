		...
		namespace Revelation
		{
			namespace Widget
			{
				...
				void PlayerStatus::create(HWND parent, uint top, uint left, uint width, uint height)
				{
					...
					string d = to_string(bases.size()) + string(" BASES\n");
					fprintf(Logfile, d.c_str());
					fclose(Logfile);
				...
				void PlayerStatus::buildMenu()
				{
					...
					for (landTerritoryIter = territoryList.begin(); landTerritoryIter != territoryList.end(); ++landTerritoryIter)
					{
						...
						string d = string(landTerritoryIter->first) + string("Start\n");
						fprintf(Logfile, d.c_str());
						fclose(Logfile);
				...
				void PrintInfoOrErrorLdwadwadwadog(string message)
				{
					FILE* Logfile = fopen("./flhook_logs/revellation.log", "at");
				...
				HK_ERROR PlayerStatus::commands(uint commandID, uint clientID)
				{
					...
					case ID_PLAYERSTATUS_CTXMENU_KILL:
						if (client.get(clientID)->kill() == HKE_OK)
						{
							client.get(clientID)->sendPrompt(
								...
								L"Вы были убиты администратором сервера."
								));
					...
					case ID_PLAYERSTATUS_CTXMENU_BAN:
						if (client.get(clientID)->ban() == HKE_OK)
						{
							client.get(clientID)->sendPrompt(
								...
								L"Вы были забанены администратором сервера."
								));
					...
					case ID_PLAYERSTATUS_CTXMENU_KICK:
						if (HkKickReason(
							...
							Language::Get()->lang(
								...
								L"Вас выкидывает с сервера администратор сервера."
							)) == HKE_OK)
						{
							return HKE_OK;
						}
					...
					default:
						PrintInfoOrErrorLdwadwadwadog(to_string(commandID) + " - ID_LOCATIONBEAM_CTXMENU_START...\n");

						if (commandID >= ID_LOCATIONBEAM_CTXMENU_START && commandID < ID_LOCATIONBEAM_CTXMENU_END)
						{
							PrintInfoOrErrorLdwadwadwadog(to_string(commandID) + " - ID_LOCATIONBEAM_CTXMENU_START commandBeam init...\n");
				...
				HK_ERROR PlayerStatus::commandCash(uint commandID, shared_ptr<Clients::Client> client)
				{
					...
					client->sendPrompt(
						...
						L"Все ваши кредиты были удалены администратором сервера."
						));

					return HkAddCash(client->getName(), 0 - iCash);
				}

				void PrintInfoOrErrorLog(string message)
				{
					FILE* Logfile = fopen("./flhook_logs/revellation.log", "at");
					string infoEx = message;
					fprintf(Logfile, infoEx.c_str());
				...
				HK_ERROR PlayerStatus::commandBeam(uint commandID, shared_ptr<Clients::Client> client)
				{
					...
					PrintInfoOrErrorLog(to_string(commandID) + "commandBeam start...\n");

					if (beamDestinations.find(commandID) == beamDestinations.end())
					{
						PrintInfoOrErrorLog(to_string(commandID) + "HKE_INVALID_BASENAME...\n");
					...
					try 
					{
						...
						{
							PrintInfoOrErrorLog(to_string(commandID) + " try HKE_INVALID_BASENAME...\n");
						...
						{
							PrintInfoOrErrorLog(to_string(commandID) + " HKE_NO_MATCHING_PLAYER...\n");
						...
						PrintInfoOrErrorLog(to_string(playerInfo.iShip) + " playerInfo.iShip...\n");
						...
						if (playerInfo.iShip != 0)
						{
							client->sendPrompt(
								...
								Language::Get()->lang(
									...
									L"Администратор сервера хочет переместить вас в ${BASE}" \
									L". Но это возможно только тогда, когда вы не находитесь в космосе."
								),
								assign
								);

							MessageBox(widget, "Игрок должен приземлиться на базу, чтобы его переместили.", "Moving player", MB_ICONWARNING);
						...
						client->sendPrompt(
							...
							Language::Get()->lang(
								...
								L"Администратор сервера переводит вас в ${BASE}. Пожалуйста, будьте готовы."
							),
							assign
							);
				...
				HK_ERROR PlayerStatus::commandFriendlyBase(shared_ptr<Clients::Client> client)
				{
					...
					if (playerInfo.iShip != 0)
					{
						MessageBox(widget, "Игрок должен приземлиться на базу.", "Base friendly", MB_ICONWARNING);
					...
					if (err == HKE_OK)
					{
						client->sendPrompt(
							...
							Language::Get()->lang(
								...
								L"Теперь вы дружите с фракцией, которая управляет базой, к которой вы в данный момент пришвартовались."
							)
							);
