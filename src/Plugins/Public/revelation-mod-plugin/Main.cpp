...
void displayHelpScreen(shared_ptr<Clients::Client> client, const wstring &wscParam)
{
	...
	if (wscParam == L"")
	{
		client->sendXMLMessage(
			L"<TRA color=\"aqua\" font=\"default\" bold=\"true\"/><TEXT>Lizerium Mod " 
...
void LoadSettings()
{
	...
	Logfile = fopen("./flhook_logs/revellation.log", "at");
	fprintf(Logfile, "1\n");
	fclose(Logfile);
	...
	//	{
		...
		//{
		//	Logfile = fopen("./flhook_logs/revellation.log", "at");
		//	fprintf(Logfile, "Can't enable widget because: YES\n");
		//	fclose(Logfile);
...
EXPORT PLUGIN_INFO* Get_PluginInfo()
{
	...
	p_PI->sName = "Revelation Mod Data Plugin by Dvurechensky";
