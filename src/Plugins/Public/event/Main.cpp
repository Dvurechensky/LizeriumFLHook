...
struct TRADE_EVENT {
	...
	int iObjectiveMax;
	int iObjectiveCurrent = 0;	// Всегда 0, чтобы не было данных
	uint uCommodityID;
	bool bLimited = false;		// Независимо от того, ограничено ли это определенным набором идентификаторов
	list<uint> lAllowedIDs;
};

struct COMBAT_EVENT {
	//Основные настройки событий
	string sEventName;
	string sURL;
	int iObjectiveMax;
	int iObjectiveCurrent = 0;		// Всегда 0, чтобы не было данных
	//Данные о боевых событиях
	bool bPlayersOnly = false;		// принимаем ложь
	list<uint> lAllowedIDs;
	...
	//Данные NPC при необходимости
	list<uint> lNPCTargetReputation;
	//Необязательные данные о товарном вознаграждении
	bool bCommodityReward = false;	// принимаем ложь
	uint uCommodityID;
};
...
struct MINING_EVENT {
	...
	string sURL;
	int iObjectiveMax;
	int iObjectiveCurrent = 0;	// Всегда 0, чтобы предотвратить отсутствие данных.
	int iBonusCash;
	//Настройки майнинга
	bool bLimited = false;		// Независимо от того, ограничено ли это определенным набором идентификаторов.
...
//арки для артиллерийских снарядов, которые разрешено минировать
set<uint> validminingarch;


//Хорошо сопоставьте идентификаторы игроков, чтобы нам не приходилось перебирать структуры игроков.
//в какой-то момент это должно быть перенесено в HookExt, чтобы все плагины могли извлечь из этого выгоду и уменьшить избыточность данных.
...
void __stdcall CharacterSelect_AFTER(struct CHARACTER_ID const & cId, unsigned int iClientID)
{
	if (HookExt::IniGetB(iClientID, "event.enabled"))
	{
		...
		{
			PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0621"), stows(mapTradeEvents[eventid].sEventName).c_str());
		}
		else
		{
			...
			PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0622"));
	...
	for (map<string, TRADE_EVENT>::iterator i = mapTradeEvents.begin(); i != mapTradeEvents.end(); ++i)
	{
		...
		if (gbi.iGoodID == i->second.uCommodityID)
		{
			...
			if (HookExt::IniGetB(iClientID, "event.enabled"))
			{
				...
				HookExt::IniSetI(iClientID, "event.eventpobcommodity", 0);
				HookExt::IniSetI(iClientID, "event.quantity", 0);
				PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0623"), stows(mapTradeEvents[eventid].sEventName).c_str());
				Notify_TradeEvent_Exit(iClientID, i->second.sEventName, "Interacted with commodity on non-event station");
			}

			//это как если да, то если игрок вышел из режима событий, он может снова войти в режим событий
			//проверяем, является ли это констатирующей точкой события
			if ((gbi.iBaseID == i->second.uStartBase) && (!HookExt::IniGetB(iClientID, "event.enabled")))
			{
				for (list<EquipDesc>::iterator item = Players[iClientID].equipDescList.equip.begin(); item != Players[iClientID].equipDescList.equip.end(); item++)
				{
					if ((mapIDs.find(item->iArchID) != mapIDs.end()) && item->bMounted)
					{
						...
						PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0624"), stows(mapIDs[item->iArchID]).c_str());
					}
				}
				...
				ConPrint(L"Register GoodBuy EVENT");
				...
				PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0625"), stows(i->second.sEventName).c_str());
				...
				if (gsi.iArchID == i->second.uCommodityID)
				{
					...
					if ((iBaseID == i->second.uEndBase))
					{
						...
						if (gsi.iCount > iInitialCount)
						{
							...
							HookExt::IniSetI(iClientID, "event.eventpobcommodity", 0);
							HookExt::IniSetI(iClientID, "event.quantity", 0);
							PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0626"), stows(i->second.sEventName).c_str());
						...
						else
						{
							...
							{
								// ничего не делать, так как базовые вознаграждения FLHook обрабатываются по-другому.
								continue;
							}
							...
							ConPrint(L"Result Trade EVENT");

							pub::Audio::PlaySoundEffect(iClientID, CreateID("ui_gain_level"));
							PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0627"), stows(i->second.sEventName).c_str());

							wstring wscCharname = (const wchar_t*)Players.GetActiveCharacterName(iClientID);

							if (i->second.iObjectiveCurrent == i->second.iObjectiveMax)
							{
								PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0628"));
							...
							else if ((i->second.iObjectiveCurrent + gsi.iCount) >= i->second.iObjectiveMax)
							{
								...
								PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0629"));
							...
							PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0630"), bonus);
					...
					else if (!i->second.bFLHookBase)
					{
						...
						HookExt::IniSetI(iClientID, "event.eventpobcommodity", 0);
						HookExt::IniSetI(iClientID, "event.quantity", 0);
						PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0631"), stows(i->second.sEventName).c_str());
...
void __stdcall SPMunitionCollision(struct SSPMunitionCollisionInfo const & ci, unsigned int iClientID)
{
	...
	{
		//PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0632"));
		return;
	}
	...
	{
		//PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0633"));
		return;
	}
	...
	if (i != mapMiningSpaceObj.end())
	{
		...
		{
			PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0634"));
			return;
		}

		if (eventdata.iObjectiveCurrent == 0)
		{
			PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0635"));
			return;
		}
		...
		if (eventdata.bLimited)
		{
			...
			{
				PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0636"));
				return;
			}
		}

		ConPrint(L"Register Munition EVENT");
		...
		if (iLootCount == 0)
		{
			...
			{
				PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0637"), reinterpret_cast<const wchar_t*>(Players.GetActiveCharacterName(iSendToClientID)));
				pub::Player::SendNNMessage(iClientID, CreateID("insufficient_cargo_space"));
				if (iClientID != iSendToClientID)
				{
					PrintUserCmdText(iSendToClientID, GetLocalized(iClientID, "MSG_0638"));
	...
	for (map<uint, EVENT_PLUGIN_POB_TRANSFER>::iterator iter = transfermap.begin(); iter != transfermap.end(); iter++)
	{
		...
		if (HookExt::IniGetB(iter->first, "event.enabled"))
		{
			...
			if (mapTradeEvents.find(iter->second.eventid) != mapTradeEvents.end())
			{
				...
				{
					PrintUserCmdText(iter->first, GetLocalized(iter->first, "MSG_0628"));
				...
				else if ((mapTradeEvents[iter->second.eventid].iObjectiveCurrent + iter->second.count) >= mapTradeEvents[iter->second.eventid].iObjectiveMax)
				{
					...
					PrintUserCmdText(iter->first, GetLocalized(iter->first, "MSG_0629"));
				...
				PrintUserCmdText(iter->first, GetLocalized(iter->first, "MSG_0642"), stows(mapTradeEvents[iter->second.eventid].sEventName).c_str());
				PrintUserCmdText(iter->first, GetLocalized(iter->first, "MSG_0643"), bonus);
...
void SendDeathMsg(const wstring &wscMsg, uint iSystem, uint iClientIDVictim, uint iClientIDKiller)
{
	...
	if (victim)
	{
		if (HookExt::IniGetB(iClientIDVictim, "event.enabled"))
		{
			...
			PrintUserCmdText(iClientIDVictim, GetLocalized(iClientIDVictim, "MSG_0644"), stows(mapTradeEvents[sIDVictimEvent].sEventName).c_str());
		}
	...
	if (victim && killer)
	{
		...
		for (map<string, COMBAT_EVENT>::iterator i = mapCombatEvents.begin(); i != mapCombatEvents.end(); ++i)
		{
			...
			if (i->second.iObjectiveCurrent == i->second.iObjectiveMax)
			{
				PrintUserCmdText(iClientIDKiller, GetLocalized(iClientIDKiller, "MSG_0645"));
			}
			else
			{
				...
				if (bFoundSystem)
				{
					...
					if (bFoundIDKiller)
					{
						...
						if (bFoundIDVictim)
						{
							...
							{
								i->second.iObjectiveCurrent = i->second.iObjectiveMax;
								PrintUserCmdText(iClientIDKiller, GetLocalized(iClientIDKiller, "MSG_0646"));
							...
							//После того как мы обновили статус, обрабатываем награду
							// Предоставить товарное вознаграждение, если выбрано
							if (i->second.bCommodityReward == true)
							{
								//TODO
								break;
							}
							//Еще предоставить денежное вознаграждение
							else
							{
								...
								PrintUserCmdText(iClientIDKiller, GetLocalized(iClientIDKiller, "MSG_0647"), i->second.bonusplayer, i->second.iObjectivePlayerReward);
