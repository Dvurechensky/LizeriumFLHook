...
void DoHashList(CCmds* cmds)
{
	char* szFLConfig = (char*)((char*)GetModuleHandle(0) + ADDR_FLCONFIG);
	HkLoadDLLConf(szFLConfig, cmds);
	cmds->Print(L"Start Do Hash Process... " + std::to_wstring(lstFLPaths.size()) + L"|" + std::to_wstring(vDLLs.size()) + L"\n");
	DoHashListLogic(cmds);
	cmds->Print(L"Stop Do Hash Process...\n");
}
...
void DoHashListLogic(CCmds* cmds)
{
	...
	if (!pGetBuf)
	{
		cmds->Print(L"Error Get Process Address");
		return;
	}
...
void HkLoadDLLConf(const char *szFLConfigFile, CCmds* cmds)
{
	...
	{
		cmds->Print(L"Load-" + std::to_wstring(i) + L"\n");
		FreeLibrary(vDLLs[i]);
	}
	vDLLs.clear();
