...
wstring GetLocation(unsigned int iClientID)
{
	...
	if (!iSystemID || !iShip)
	{
		PrintUserCmdText(iClientID, GetLocalized(iClientID, "MSG_0001"));
		return false;
	}
...
wstring GetTimeString(bool bLocalTime)
{
	...
	wchar_t wszBuf[100];
	//_snwprintf_s(wszBuf, sizeof(wszBuf), L"%04d.%02d.%02d [%02d:%02d:%02d] ", st.wYear, st.wMonth, st.wDay,
	//	st.wHour, st.wMinute, st.wSecond);
	_snwprintf_s(wszBuf, sizeof(wszBuf), L"[%02d:%02d:%02d] ", st.wYear, st.wMonth, st.wDay,
	st.wHour, st.wMinute, st.wSecond);
	return wszBuf;
}
...
void PrintLocalUserCmdText(uint iClientID, const wstring &wscMsg, float fDistance)
{
	...
	while (pPD = Players.traverse_active(pPD))
	{
		...
		PrintUserCmdText(iClientID2, GetLocalized(iClientID, "MSG_0002"), wscMsg.c_str());
	}
}

// Print message to all ships within the specific number of meters of the player.
void PrintLocalUserCmdColorText(uint iClientID, 
	const wstring& timeMsg,
	const wstring& typeMsg,
	const wstring& playerNameMsg,
	const wstring& wscMsg, 
	float fDistance)
{
	uint iShip;
	pub::Player::GetShip(iClientID, iShip);
	...
	while (pPD = Players.traverse_active(pPD))
	{
		...
		PrintUserCmdTextColor4KV(iClientID2, timeMsg, typeMsg, playerNameMsg, wscMsg);
	}
}
